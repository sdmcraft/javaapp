...
public class CrudeTCPServer
{
    ...
    public void start() throws Exception
    {
        try
        {
            keyPair = EncryptUtils.generateKeyPair("keys/public.key", "keys/private.key");
            System.out.println("Server ready to accept connections");

            while (true)
            {
                Socket socket = serverSocket.accept();
                String connectionName = "Connection:" + connectionMap.size() + 1 + "@" + System.currentTimeMillis();
    ...
    private class Connection implements Runnable
    {
        ...
        private Connection(Socket socket, String name, CrudeTCPServer server) throws IOException
        {
            this.socket = socket;
            this.name = name;
            this.server = server;
        ...
        private void messageInterpreter(String message) throws Exception
        {
            ...
            if ("Let's get cryptic".equalsIgnoreCase(message))
            {
                ...
                String encryptedMessage = new String(EncryptUtils.rsaEncrypt(serverMessage, keyPair.getPrivate()));
