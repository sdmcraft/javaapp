...
public class Matrix
{
    ...
    public static Matrix buildConnected(int numRows, int numCols, double factor)
    {
        ...
        while (!(matrix = Matrix.cycleFreeBuild(numRows, numCols, factor)).isConnected())
        {
            ;
        }
        ...
        do
        {
            ...
            for (int row = 0; row < data.length; row++)
            {
                for (int col = 0; col < data[0].length; col++)
                {
                    ...
                    {
                        boolean newValue = tempMatrix.getBoolean(row, col) || tempMatrix.getBoolean(col, row) || (tempMatrix.getBoolean(row, pointer) && tempMatrix.getBoolean(pointer, col));
