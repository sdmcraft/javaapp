...
public class Tree implements Cloneable, Serializable
{
    ...
    private void getDiagram(Tree root)
    {
        ...
        {
            diagram += ("\"" + root.nodeID + "\"" + "->" + "\"" + root.sibling.nodeID + "\"" + "[constraint=false];\n");
        }
            ...
            for (Tree child : root.getChildren())
            {
                ...
                if (root instanceof BinaryTree)
                {
                    ...
                    if (btChild.getLeft() == null)
                    {
                        ...
                        diagram += ("\"" + child.nodeID + "\"" + "->" + "\"" + "null" + terminalCount + "\" [color=green];\n");
                        terminalCount--;
                    }
                    ...
                    {
                        diagram += ("null" + terminalCount + " [shape=point];\n");
                        diagram += ("\"" + child.nodeID + "\"" + "->" + "\"" + "null" + terminalCount + "\" [color=red];\n");
                        terminalCount--;
                    }
                }

                diagram += ("\"" + root.nodeID + "\"" + "->" + "\"" + child.nodeID + "\"" + ((color != null) ? ("[color=" + color + "]") : "") + ";\n");
                getDiagram(child);
            }
        }
    }
    ...
    public static Tree generate(int levels, int maxVal, int maxChildren)
    {
        ...
        {
            Tree tree = new Tree(Integer.toString((int) (Math.random() * maxVal)));
