...
public class FirstLoginModule implements LoginModule {
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler,
		...
		System.out.println("initialize called for FirstLoginModule");
	}

	@Override
	public boolean login() throws LoginException {
		System.out.println("login called for FirstLoginModule");
	...
	private boolean validate(String uname, String pwd) {
		boolean success = Info.map1.containsKey(uname + "-" + pwd);
		if (success)
			myPrincipal = new MyPrincipal(Info.map1.get(uname + "-" + pwd));
		return success;
	}
