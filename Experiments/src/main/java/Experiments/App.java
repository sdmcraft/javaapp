	...
	public static void main(String[] args) throws Exception {
		...
		try {
			Node rootNode = session.getRootNode();
			 ...
			 exportToXML(new File("repositoryXML.xml"), rootFolderNode,
			 session);
			 rootFolderNode.remove();
//			session.importXML(rootNode.getPath(), new FileInputStream(new File(
//					"repositoryXML.xml")),
//					ImportUUIDBehavior.IMPORT_UUID_CREATE_NEW);
	...
	public static void exportToXML(File file, Node node, Session session)
		...
		try {
			...
			session.exportSystemView(node.getPath(), out, false,
					false);
		} finally {
			if (out != null)
				out.close();
