...
$('document')
		.ready(
				function() {
					...
					// this loop moves all the light pieces to their initial
					// positions
					$('div.piece.light').each(
							function(index, piece) {
								...
								// turning the x,y coordingate into a pixel
								// position
								var pixelPosition = getPixels(x, y);

								// YOUR CODE LAST WEEK
								// actually moving the piece to its initial
								// position
								movePieceTo($(piece), pixelPosition.top,
										pixelPosition.left);
							});

					// this loop moves all the dark pieces to their initial
					// positions
					$('div.piece.dark').each(
							function(index, piece) {
								...
								// turning the x,y coordinate into a pixel
								// position
								var pixelPosition = getPixels(x, y);
					...
					$('div.piece')
							.click(
									function() {
										...
										// YOUR CODE LAST WEEK
										// toggleing the 'selected' class of
										// this piece
										...
										if ($this.hasClass('selected')
												&& ((window.turn === 'light' && $this
														.hasClass('light')) || (window.turn === 'dark' && $this
														.hasClass('dark')))) {
											getMovableSquares($this).addClass(
													'movable');
										}
					...
					$('div.square')
							.click(
									function() {
										...
										// if $this is a legal square to move
										// to...
										if ($this.hasClass('movable')) {

											// get the piece with the class
											// 'selected'
											var $selectedPiece = $('div.piece.selected');

											// we only move if there is exactly
											// one selected piece
											if ($selectedPiece.length == 1) {
												// get the index of the square
												// and translate it to pixel
												// position
...
function changeTurnDiplay() {
	$("#turnDisplay").html(window.turn);
}
		...
		$.each(vectors, function(index, vector) {
			...
			if (takenSquares[newSquareIndex]) {
				...
				if ($piece.hasClass('dark')) {
					if (takenSquares[newSquareIndex].hasClass('dark'))
						return;
				} else {
					if (takenSquares[newSquareIndex].hasClass('light'))
						return;
				}
				...
				if (!takenSquares[jumpSquareIndex]) {
					...
					if (!$newSquare.is($legalSquares)) {
						...
						if ($currentSquare.data('jumpedPieces')) {
							$jumpedPieces = $jumpedPieces.add($currentSquare
									.data('jumpedPieces'));
						}
